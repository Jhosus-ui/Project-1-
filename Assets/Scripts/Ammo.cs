...
public class AmmoBox : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player"))
        {
            ...
            {
                int randomAmmoAmount = Random.Range(minAmmoAmount, maxAmmoAmount + 1); // Genera un valor aleatorio entre minAmmoAmount y maxAmmoAmount
                AmmoManager.Instance.AddAmmo(randomAmmoAmount); //Checkear el respawn
                Destroy(gameObject);
            }
        }
    }
